...
const collisionPyompy = (Player1: Player, Player2: Player): void => {
  if (
  ...
  const dist_sq = dist_x ** 2 + dist_y ** 2;
  const dist_trigger = Player2.rayon + Player1.rayon;
  const dist_trigger_sq = dist_trigger ** 2;

  if (dist_sq < dist_trigger_sq) {
    const dist = dist_sq ** 0.5;
    ...
    const velo_tt_l = (velo_x_tt ** 2 + velo_y_tt ** 2) ** 0.5;
    if (velo_tt_l === 0) return;
    ...
    const ratio = Math.abs(
      (velo_x_tt / velo_tt_l) * c + (velo_y_tt / velo_tt_l) * s
    );
    ...
    const velo_tt_l2 = (velo_x_tt2 ** 2 + velo_y_tt2 ** 2) ** 0.5;
